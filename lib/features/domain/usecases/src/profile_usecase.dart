...
class ProfileUsecase extends BaseUseCase<List<ProfileEntity>, String> {
  ...
  Future<Either<Exception, List<ProfileEntity>>> call(String userId) async {
    try {
      ...
      if (response != null && response.isSuccessResponse && response.data != null) {
        return Right(response.data!);
      } else {
        return Left(Exception(response?.message ?? 'Failed to load profile scores'));
      }
  ...
  Future<Either<Exception, List<RankUserEntity>>> getProfileRank() async {
    try {
      ...
      if (response != null && response.isSuccessResponse && response.data != null) {
        ...
        return Left(Exception(response?.message ?? 'Failed to load profile rank'));
      }
    } on Exception catch (error) {
      return Left(error);
    }
